...
const projects = [
  ...
  },
  {
    ...
    desc: 'MCP (Model Context Protocol) Server untuk analisis gambar menggunakan vision AI model (mimo-v2.5-free).',
    tech: 'Python / MCP',
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {projects.map((proj, idx) => (
            <motion.a
              ...
              className={`block relative bg-p5-dark border-2 border-p5-gray p-6 lg:p-8 hover:bg-p5-white hover:border-p5-red group transition-all duration-300 ${proj.link !== '#' ? 'cursor-pointer' : 'cursor-default'}`}
            >
              {/* Corner accent */}
              <div className="absolute top-0 right-0 w-16 h-16 bg-p5-red clip-angle-reverse opacity-0 group-hover:opacity-100 transition-opacity" />

              <div className="flex justify-between items-start mb-6 relative z-10">
                <h3 className="text-3xl font-black text-p5-white group-hover:text-p5-black tracking-tighter uppercase break-words pr-2">
                  {proj.title}
                </h3>
                {proj.hasLogo && (
                  <img src="/krafton-logo.png" alt="Krafton" className="h-10 w-auto grayscale group-hover:grayscale-0 transition-all flex-shrink-0" />
                )}
                {!proj.hasLogo && proj.link !== '#' && (
                  <svg className="w-8 h-8 text-p5-gray group-hover:text-p5-red transform group-hover:translate-x-1 group-hover:-translate-y-1 transition-all" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <p className="text-p5-gray group-hover:text-p5-dark text-base font-bold mb-8 line-clamp-4">
                {proj.desc}
              </p>

              <div className="absolute bottom-6 left-6 inline-block bg-p5-black text-p5-red font-mono font-bold px-3 py-1.5 border border-p5-red group-hover:bg-p5-red group-hover:text-p5-white text-sm">
                {proj.tech}
              </div>
            </motion.a>
          ))}
